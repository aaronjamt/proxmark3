...
static int seos_challenge_get(uint8_t *RNDICC, uint8_t RNDICC_len, uint8_t keyslot) {
    ...
    snprintf(getChallengePre + strlen(getChallengePre), 3, "%02u", keyslot & 0xF);
...
static int seos_pacs_adf_select(char *oid, int oid_len, uint8_t *data_tag, int data_tag_len, int key_index, uint8_t keyslot, uint8_t *write, size_t write_len) {
    int resplen = 0;
    uint8_t response[PM3_CMD_DATA_SIZE];
    bool activate_field = false;
    bool keep_field_on = true;
    ...
    if (ALGORITHM_INFO_value1 == 0x09 || ALGORITHM_INFO_value1 == 0x02) {
        ...
        if (write) {
            ...
            if (sio_buffer_out[0] == 0x30) {
                PrintAndLogEx(SUCCESS, "SIO.............................. "_YELLOW_("%s"), sprint_hex_inrow(sioOutput, sio_size));                      // SIO
                ...
                PrintAndLogEx(SUCCESS, "Raw Value........................ "_YELLOW_("%s"), sprint_hex_inrow(sio_buffer_out, sio_size));                 // Raw Data
            }
        }
    } else {
        PrintAndLogEx(ERR, "Unknown encryption algorithm");
...
static int seos_gdf_select(int key_index) {
    ...
    seos_challenge_get(RNDICC, sizeof(RNDICC), 0x02);
...
static int seos_pacs(char *oid, int oid_len, uint8_t *data_tag, int data_tag_len, int key_index, uint8_t keyslot, uint8_t *write, size_t write_len) {
    int res = seos_aid_select();
    if (res != PM3_SUCCESS) {
        DropField();
        return res;
    ...
    res = seos_pacs_adf_select(oid, oid_len, data_tag, data_tag_len, key_index, keyslot, write, write_len);
    DropField();
    return res;
}
...
static int CmdHfSeosPACS(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf seos pacs",
                  ...
                  "hf seos pacs\n"
                  "hf seos pacs --ki 1\n"
                  "hf seos pacs -o 2B0601040181E438010102011801010202 --ki 0 --ks 1\n"
                 );
    ...
    void *argtable[] = {
        ...
        arg_int0(NULL, "ki", "<dec>", "Specify key index to set key in memory"),
        arg_int0(NULL, "ks", "<dec>", "Specify keyslot to authenticate against (Default: 1)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    int keyslot = arg_get_int_def(ctx, 3, 1);
    ...
    return seos_pacs((char *)oid, oid_len, data_tag, data_tag_len, key_index, keyslot, 0, 0);
}
...
static int CmdHfSeosWrite(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf seos write",
                  ...
                  "hf seos write -o 2B0601040181E438010102011801010202 --ki 0 -d 12345678\n"
                 );

    void *argtable[] = {
        arg_param_begin,
        arg_str0("o", "oid", "<hex>", "<0-100> hex bytes for OID (Default: 2B0601040181E438010102011801010202)"),
        arg_str0("t", "tag", "<hex>", "<0-100> hex bytes for tag to write (Default: FF00)"),
        arg_str0("d", "data", "<hex>", "<0-128> hex bytes for data (Must be valid BER-TLV)"),
        arg_int0(NULL, "ki", "<dec>", "Specify key index to set key in memory"),
        arg_int0(NULL, "ks", "<dec>", "Specify keyslot to authenticate against (Default: 2)"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    CLIGetHexWithReturn(ctx, 2, data_tag, &data_tag_len);

    int data_len = 0;
    uint8_t data[256] = {};
    CLIGetHexWithReturn(ctx, 3, data, &data_len);

    int key_index = arg_get_int_def(ctx, 4, 0);
    int keyslot = arg_get_int_def(ctx, 5, 2);
    ...
    if (data_tag_len == 0) data_tag_len = 2;
    ...
    return seos_pacs((char *)oid, oid_len, data_tag, data_tag_len, key_index, keyslot, data, data_len);
}
...
static int CmdHfSeosADF(const char *Cmd) {
    ...
    int keyslot = arg_get_int_def(ctx, 4, 1);
    ...
    return seos_pacs((char *)oid, oid_len, data_tag, data_tag_len, key_index, keyslot, NULL, 0);
}
